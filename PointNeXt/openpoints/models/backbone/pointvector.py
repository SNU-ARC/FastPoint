...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, get_aggregation_feautres, ball_query, find_mps, multi_level_filtering, fused_ball_query, fused_convert_ball_query, extract_ball_query, update_distance, remainder_fps, quick_fps, edgepc_sample
...
class LocalAggregation(nn.Module):
    ...
    def __init__(self,
        ...
        self.vpsa=PATM(channels[0],channels[-1],flag=1)
...
class SetAbstraction(nn.Module):
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            dp, fj = self.grouper(new_p, p, f, nns_idx)
...
class SetAbstractioncls(nn.Module):
    ...
    def __init__(self,
        ...
        if self.all_aggr :
            ...
            self.convs=PATM(channels[0]-3,channels[-1],self.flag)
        if not is_head:
            if self.all_aggr:
                group_args.nsample = None
                group_args.radius = None
