...
from ..layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
    create_grouper, furthest_point_sample, random_sample, three_interpolation, get_aggregation_feautres, ball_query, find_mps, multi_level_filtering, fused_ball_query, fused_convert_ball_query, extract_ball_query, update_distance, remainder_fps
...
def multi_level_lfps(p, nsample, dataset, radius, model):
...
class SetAbstraction(nn.Module):
    ...
    def __init__(self,
        ...
        if not is_head:
            ...
            if sampler.lower() == 'fps':
            ...
            elif sampler.lower() == 'lfps' or sampler.lower() == 'fused':
                ...
                self.sample_fn = partial(multi_level_lfps, radius=self.radius, model=self.model_list)
    ...
    def forward(self, pf):
        ...
        if self.is_head:
            ...
            if not self.all_aggr:
                if self.sampler.lower() == 'lfps' or self.sampler.lower() == 'fused':
...
class PointNextEncoder(nn.Module):
    ...
    def forward_seg_feat(self, p0, f0=None, dataset=None):
        ...
        for i in range(0, len(self.encoder)):
            if i == 1:
                _p, _f = self.encoder[i]([p[-1], f[-1], dataset])
            else:
