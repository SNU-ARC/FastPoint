...
class S3DIS(Dataset):
    ...
    def __init__(self,
        ...
        if split == 'train':
            ...
            self.data_list = [
                item for item in data_list if 'Area_{}'.format(test_area) not in item and 'Area_{}'.format(test_area + 1) not in item]
        elif split == 'train-val':
            self.data_list = [
                item for item in data_list if 'Area_{}'.format(test_area + 1) in item]
    ...
    def __getitem__(self, idx):
        ...
        #sort_idx = np.argsort(coord[:, 0])
        #coord, feat, label = coord[sort_idx], feat[sort_idx], label[sort_idx]
